...
## Step 3: Extract Design Assets (optional, manual handoff)

Ask the user whether they want to process design assets (background images, SVGs, icons). Default: skip.

This skill does not execute asset download commands. When the user opts in, hand off to `athenai-assets` and pass layer URLs from `get_design_context` output.
...
### 3b. Preserve asset references

- Build stable target paths under `drafts/media/` for each classified asset.
- If matching files already exist under `drafts/media/`, reuse those paths.
- Record the paths in design output so references are not lost, even when binaries are downloaded later.
- Include a short handoff note in the report: `Run athenai-assets with these layer URLs to materialize binaries.`
