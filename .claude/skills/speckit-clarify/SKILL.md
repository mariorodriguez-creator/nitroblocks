...
- **Spec–Design Alignment** (when design.md exists): Gaps or conflicts between spec.md and design.md — e.g. spec describes components/variants/behaviors not reflected in design.md; design shows elements (block-scoped class names, breakpoints, states) not addressed in spec; missing author-configurable mapping; unclear dynamic content behavior. Also check that design variants map to block options defined in the content model.
- **Variant Completeness** (when design.md exists): When design.md mentions variant classes for colour or visual overrides (EDS-style: e.g. `.blockname.dark`, `.blockname.blockname-reversed`; strict BEM `--` modifiers only if the project uses them), it MUST include the actual CSS rules—either in the CSS Skeleton or as concrete values (color, background-color, border-color). If design.md only names a variant class without specifying what those overrides are, flag as ambiguous. Ask the user to clarify the variant colours/values so they can be added to spec or design.
